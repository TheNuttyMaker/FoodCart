//https://foodkart-3c10a.firebaseapp.com/__/auth/handler              FB firebase handler
//https://www.facebook.com/connect/login_success.html
//http://localhost:8100/oauthcallback.html
angular.module('app.controllers', [])

.controller('loginCtrl', function($scope,$rootScope,$ionicHistory,sharedUtils,$state,$ionicSideMenuDelegate, ngFB) {
        ...
        function (response) {
            if (response.status === 'connected') {
                ...
                firebase.auth().signInWithCredential(credential).then(function(result) {
                    console.log("sined fb result"+ JSON.stringify(result));
                    ...
                    // ...
                    sharedUtils.hideLoading();
                    console.log("sined fb"+ errorCode + "errorMessage  "+ errorMessage);
                    sharedUtils.showAlert("Please note"+errorMessage);
                });
...
/*
    $scope.loginFb = function(){
      ...
      console.log("sined fb `");
      firebase.auth().signInWithRedirect(provider);
      console.log("sined fb 2");
      sharedUtils.showLoading();
    ...
    $scope.signupEmail = function (formName, cred) {
      ...
      if (formName.$valid) {  // Check if the form data is valid or not
        ...
        firebase.auth().createUserWithEmailAndPassword(cred.email, cred.password).then(function (result) {
            var pic = "https://scontent.xx.fbcdn.net/v/t1.0-1/p100x100/11402933_10206225905399245_1323506916467819568_n.jpg?oh=a06f233f4c5b6e9a4e5f1c5155879af5&oe=58DAA4AA";
  ...
  firebase.auth().onAuthStateChanged(function(user) {
    if (user) {
      $scope.user_info=user; //Saves data to user_info
            //Add photo to the user table
            fireBaseData.refUser().child($scope.user_info.uid).set({
              photoURL: $scope.user_info.user.photoURL
            });
    }else {
